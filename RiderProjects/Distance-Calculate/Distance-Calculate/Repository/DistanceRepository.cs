...
public class DistanceRepository : IDistanceRepository
{
    ...
    public DistanceRepository(WebAPIContext context)
    {
        _context = context;
    }
    ...
    public async Task<Object> CalculateDistanceSpeedTime(int userId, double latitude, double longitude)
    {
        try
            {
                        ...
                        else
                        {
...

                            {
                               ...
                               // Lưu kết quả tính toán vào cơ sở dữ liệu
                                ...
                                // Tính trung bình cộng của các trường distance, speed và time
                                double TotalDistanceFromStart = await _context.CalculationResult.Where(c => c.UserId == userId)
            ...
            catch (Exception)
            {
                ...
                return (new { message = "UserId does not exist" });
